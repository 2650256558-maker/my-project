...
for (FLAG, curr_train_ratio,Scale) in [(1,0.01,100)]:
# for (FLAG, curr_train_ratio,Scale) in [(2,0.01,100),(3,0.01,100)]:
    ...
    for curr_seed in Seed_List:
        ...
        # 获取 FLOPs (你需要提供一个示例的输入张量)
        dummy_input = torch.randn(height, width, bands).to(device)
        print_flops(net, dummy_input)
        ...
        with torch.no_grad():
            ...
            pltgraph(gt,output,'Botswana',dataset_name)
            print('------------------------graph-----------------------')
