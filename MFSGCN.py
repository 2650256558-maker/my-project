...
class PGMN(nn.Module):
    def __init__(self, input: int, output: int,droupout,A,res=False,MPNN='GCN'):
        ...
        if MPNN=='GCN':
            ...
            if A.is_sparse:
                A = A.coalesce()  # 确保索引规范化
                self.edge_index = A.indices()  # shape: [2, num_edges]
                self.edge_weight = A.values()  # shape: [num_edges]
            else:
                A = A.cpu()
...
class LSSF(nn.Module):
    '''
    Spectral-Spatial Convolution
    '''

    def __init__(self, in_ch, out_ch, kernel_size=3):
        super(LSSF, self).__init__()
...
class MFSGCN(nn.Module):
    def __init__(self, height: int, width: int, changel: int, class_count: int, Q: torch.Tensor, A: torch.Tensor,
        ...
        for i in range(layers_count):
            if i < layers_count - 1:
                self.CNN_Branch.add_module('CNN_Branch' + str(i), LSSF(128, 128, kernel_size=5))
            else:
                self.CNN_Branch.add_module('CNN_Branch' + str(i), LSSF(128, 64, kernel_size=5))
        ...
        for i in range(gnncout):
            if i < gnncout - 1:
                self.GCN_Branch.add_module('GCN_Branch' + str(i), SSAN(128, 128, self.A,0))
            else:
                self.GCN_Branch.add_module('GCN_Branch' + str(i), SSAN(128, 64, self.A,0))


        PGMNlayers_count = 2  # ,2,
        self.PGMN_Branch = nn.Sequential()
        for i in range(PGMNlayers_count):
            print('PGMN')
            if i < PGMNlayers_count - 1:
                self.PGMN_Branch.add_module('GCN_Branch' + str(i), PGMN(input=128,output= 128,droupout=0.8,A=self.AX))
            else:
                self.PGMN_Branch.add_module('GCN_Branch' + str(i), PGMN(input=128,output= 64,droupout=0.8,A=self.AX))
    ...
    def forward(self, x: torch.Tensor):
        ...
        PGMNruslt=self.PGMN_Branch(clean_x_flatten)
        ...
        #GCN_result = torch.matmul(self.Q, H)+PGMNruslt
        GCN_result = self.decode(CNN_result, H, self.Q)+PGMNruslt
